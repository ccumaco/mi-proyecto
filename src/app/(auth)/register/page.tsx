...
export default function RegisterPage() {
  ...
  const handleStep3 = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 1. Check if we have an authenticated user (from Step 1)
    const { data: { user } } = await supabase.auth.getUser();
    
    if (!user) {
      // If user hasn't confirmed email yet, we might need to handle this
      // For now, let's proceed with the success screen, but in a real app
      // you'd persist this data after confirmation.
      setStep(4);
      return;
    }
    ...
    try {
      ...
      // 3. Create the Units for each Tower
  ...
  const selectClass =
    'w-full rounded-xl border-2 border-zinc-100 bg-white px-4 py-3 pl-11 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 dark:border-zinc-800 dark:bg-zinc-900/50 dark:text-white';
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Right: Form */}
      <div className="flex flex-1 items-center justify-center bg-white p-6 sm:p-10 overflow-y-auto">
        <div className="w-full max-w-lg space-y-8">
              ...
              <form onSubmit={handleStep2} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-semibold text-zinc-700">País</label>
          ...
          {step === 3 && (
            <>
              ...
              <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 flex items-start gap-2 text-sm text-zinc-700">
              ...
              <form onSubmit={handleStep3} className="space-y-6">
                {towers.map((tower) => {
                  ...
                  return (
                    <div
                      key={tower.id}
                      className="rounded-xl border-2 border-zinc-200 overflow-hidden"
                    >
                      <button
                        type="button"
                        onClick={() => setExpandedTower(isExpanded ? '' : tower.id)}
                        className="w-full flex items-center justify-between p-4 bg-zinc-50 hover:bg-zinc-100"
                      >
                        <span className="font-bold text-zinc-900">{tower.name}</span>
                      ...
                      {isExpanded && (
                        <div className="p-4 space-y-4 border-t border-zinc-200">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <label className="text-sm font-semibold text-zinc-700">Número de Pisos</label>
                              <input
                                type="number"
                                min={1}
                                value={tower.floors}
                                onChange={(e) => updateTower(tower.id, 'floors', parseInt(e.target.value, 10) || 1)}
                                className="mt-1 w-full rounded-lg border-2 border-zinc-200 px-3 py-2 text-sm"
                              />
                            </div>
                            <div>
                              <label className="text-sm font-semibold text-zinc-700">Unidades por Piso</label>
                              <input
                                type="number"
                                min={1}
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {preview.slice(0, 24).map((u) => (
                                <span
                                  key={u}
                                  className="inline-flex h-8 min-w-10 items-center justify-center rounded border border-zinc-200 bg-white px-2 text-xs font-medium text-zinc-700"
                                >
                                  {u}
                                </span>
                              ))}
                              {preview.length > 24 && (
                                <span className="inline-flex h-8 items-center px-2 text-xs text-zinc-500">
                ...
                <button
                  type="button"
                  onClick={addTower}
                  className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-zinc-200 py-4 text-sm font-medium text-zinc-500 hover:border-primary hover:text-primary"
                >
                ...
                <div className="rounded-xl border-2 border-dashed border-zinc-200 p-6 text-center">
                  ...
                  <p className="font-semibold text-zinc-900">¿Tienes muchas unidades? Ahorra tiempo</p>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm">
              <div className="flex justify-center mb-6">
                <div className="relative">
                  <div className="h-20 w-20 rounded-full bg-emerald-100 flex items-center justify-center">
                    <FontAwesomeIcon icon={faBuilding} className="h-10 w-10 text-zinc-500" />
                  </div>
              ...
              <div className="mt-6 rounded-xl bg-emerald-50 border border-emerald-100 p-4 flex items-start gap-3">
                ...
                <div className="text-sm">
                  <p className="font-semibold text-emerald-800">Prueba gratuita de 60 días iniciada</p>
